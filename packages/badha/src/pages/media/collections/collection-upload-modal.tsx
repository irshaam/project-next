...
import Dropzone, { IDropzoneProps, IFileWithMeta, ILayoutProps, IUploadParams } from "react-dropzone-uploader";
...
const InnerForm = (props: FormikProps<FormValues> & MyFormProps) => {
  ...
  const getUploadParams = ({ file, meta }: IFileWithMeta): IUploadParams | Promise<IUploadParams> => {
    const body = new FormData();
    body.append("file", file);
    body.append("collection", collection.id);
    body.append("type", "media");
    const headers = {
      // "Content-Type": "multipart/form-data",
      Authorization: `Bearer ${session?.access_token}`,
    };
    // const upload = await client.post("/media/upload", formData, {});
    return { url: `${process.env.NEXT_PUBLIC_API_URL}/media/upload`, body, headers };
  };
  ...
  const handleSubmit = (files, allFiles) => {
    const updatedFiles = files.map((f) => JSON.parse(f.xhr.response));
    props.onSubmit(updatedFiles);
